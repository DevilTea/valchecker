...
export function compareBundleImpact(baseResults, candidateResults, threshold = meaningfulBundleRegression) {
	...
	const rows = baseResults.map((base) => {
		...
		return {
			...
			deltaBytes: candidate.brotliBytes - base.brotliBytes,
			ratio,
			delta,
	...
	return {
		threshold,
		verdict: regressions.length > 0 ? 'regression' : improvements.length > 0 ? 'improvement' : 'neutral',
		rows,
