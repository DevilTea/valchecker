...
function renderHtml(raw) {
	const metadata = metadataRows(raw)
		...
		.join('')
	const sections = raw.libraries[0].results.map(result => result.scenario).map((scenario) => {
		const rows = scenarioRows(raw, scenario)
		...
		return `<section><h2>${htmlEscape(scenario)}</h2><p>Category: <strong>${htmlEscape(rows[0].category)}</strong></p><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Library</th><th>Version</th><th>Median ops/s</th><th>Median ns/op</th><th>Fastest</th><th>vs Valchecker</th><th>RME</th></tr></thead><tbody>${body}</tbody></table></div></section>`
	}).join('')
