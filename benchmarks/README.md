...
**A triggered group is confirmed too, on one runner.** A severe group verdict blocks, so it is held to the standard a severe cell already had. When a group triggers, the confirmation batch measures **every** cell of that group — not only the boundary cells the row selection would pick — and runs unsharded, because a group aggregate mixed across runners is the defect being corrected and confirming it on four machines would reproduce it. The group blocks only if that single-runner batch also classifies it a regression. Everything that decision rests on is read from the artifacts rather than passed between jobs: if the confirmation ran sharded, or did not measure the whole group, the trigger is `review` whatever the workflow intended. A confirmation group aggregate over a group it did not measure in full is never read — that would be an aggregate over an outcome-selected subset, the bias the group estimator exists to remove. The `verdict` job carries the exit code and `compare` reports. Group triggers are confirmed by their own single-runner batch, described above.
