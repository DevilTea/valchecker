...
The **Bundle Size Impact** workflow is a direct base-versus-candidate consumer bundle comparison. On a pull request it builds both revisions, packs `@valchecker/internal`, `@valchecker/all-steps`, and `valchecker` from each revision with `pnpm pack`, installs each three-tarball set into its own temporary consumer, and then bundles both sides with the **same checked-out benchmark harness, Rollup, Terser, and scenario source**. The cells are therefore an impact measurement of the package artifacts rather than two measurements produced by two revisions of the measuring tool.

The packed consumer graph is checked, not assumed. `valchecker`'s `@valchecker/internal` and `@valchecker/all-steps` imports must resolve to the exact same physical tarball installations supplied for that side; a registry copy with the same version is rejected. The report records the packed manifests used for both revisions.

Published package metadata is the consumer contract. The primary profile reads each installed package's `sideEffects` declaration and converts that declaration into Rollup's per-module `moduleSideEffects` answer. It does **not** force `moduleSideEffects: false`, `propertyReadSideEffects: false`, or Terser's `pure_getters: true`. Missing `sideEffects` metadata is conservative, precise string/glob exceptions are honored, and malformed metadata fails closed. `pnpm test:package` separately requires all three current packed Valchecker manifests to preserve the repository's intended `sideEffects: false` promise.

For every Valchecker bundle scenario, the impact table reports base Brotli bytes, candidate Brotli bytes, byte delta, percentage delta, and classification. Brotli is deterministic under this fixed toolchain — a base=base run produces zero bytes of difference in all scenarios — so the **5% threshold is a review/blocking policy, not a noise band**. Every delta is visible; a candidate Brotli increase greater than 5% is a bundle regression and the pull-request workflow passes `--fail-on-regression`. This uses the same 5% meaningful-change boundary already used by the benchmark policy rather than treating a one-byte change as an automatic failure.

An intentional feature or correctness change may exceed that policy only through a **base-commit-pinned byte acknowledgement** in `src/accepted-bundle-regressions.mjs`. The entry names the exact historical base, affected scenarios, and a maximum Brotli-byte increase. Within that ceiling the row is reported as `accepted-regression`; exceeding it still fails. Once `main` advances, the acknowledgement no longer applies, so it cannot silently raise the budget for later pull requests.

The existing current-build checks remain, but under their correct name: **structural tree-shaking guardrails**. They execute realistic candidate bundles, require selective builds to remain materially smaller than default/full builds, and verify required/forbidden Valchecker markers. These checks answer whether the candidate is still tree-shakable; they cannot answer how much the pull request changed bundle size and therefore do not substitute for the base/candidate impact table. Cross-library Zod/Valibot sizes are candidate-side context only.

Valchecker is still measured in two public usage modes:
...
The workflow artifact contains concise/detailed Markdown, HTML, JSON evidence, and generated base/candidate bundles. A current-only local run is useful for the structural guardrails but explicitly makes **no impact claim**:
...
For an impact comparison, build both workspaces first and provide both roots:

```bash
pnpm --dir benchmarks treeshake \
  --output ../artifacts/bundle-size-impact \
  --base-root /path/to/built-base \
  --candidate-root /path/to/built-candidate \
  --base-commit <base-sha> \
  --candidate-commit <candidate-sha> \
  --fail-on-regression
```

Bundle size is independent from runtime throughput; use the Performance Impact and cross-library runtime suites for execution behavior.
...
**A canary set runs whatever the diff says:** every `construction` and `cold` cell, plus thirteen named core cells. That is 18 cells today: the five construction/cold cells are taken whole because module initialisation, step registration, and the shared prototype shape are not attributable through execution cells, while the named cells cover the per-call floor, common structure walks, issue construction, deferred messages, collect-all traversal, and async machinery. They are **health controls**, not evidence that a scoped product group broadly regressed. `selection.json` labels every measured cell `affected` or `health-canary`, the raw measurement identity carries that role map, and the comparison refuses runs whose roles differ. A canary row keeps its own direct verdict — a severe canary is still a real signal — but it never enters a scoped group aggregate merely because it ran.

**A severe-group trigger needs at least two affected cells, not two cells that happened to run.** The old selector topped a thin group up with unrelated cells, and the canary itself could make a group look adequately sampled; both were statistically wrong once the aggregate is understood as "did this affected group broadly regress?". Scoped selection now contains only the diff-attributed cells plus health canaries and never adds an unaffected top-up. If a group has one affected cell, its direct row verdict still applies but no group verdict is manufactured from a one-row aggregate. The Scenario scope report shows measured cells and affected estimator cells separately, and names groups whose group trigger is unavailable.

**A group is estimated directly over the affected cells only.** Per repetition, the log ratios of every **diff-attributed** cell in the group are averaged into `G_r`; the group's change is `exp(mean(G)) - 1` and its verdict is the Student-t interval across `G_1 ... G_5`, severe when that whole interval is at or below -5%. A cell verdict answers "did this cell regress?" and the group answers "did this affected group broadly regress?". Health canaries are intentionally outside that estimator: two affected cells at -6% plus two flat canaries must still estimate the group at -6%, not about -3%. Within the affected set, an inconclusive row still contributes — filtering by its measured outcome would bias the estimate — and a group can be decisive where none of its rows is because averaging within a repetition can cancel cell-specific noise.

**A group aggregate carries both of its denominators.** The report distinguishes `affected`, `health`, and total catalog membership. The estimate is over `affected`; `measured` is the larger set that also contains health controls. That distinction is load-bearing: a scoped run may measure several canaries in a group while the diff reaches only one product cell, and presenting `measured/total` alone would make a one-cell estimator look like a multi-cell group sample. A partly covered run therefore states exactly how many affected cells produced the aggregate and how many additional cells were measured only as health signals.

There is deliberately no percentage coverage floor for an affected estimator: the cells omitted by scoping are the ones the import attribution says the diff cannot move, and the conservative fallback for anything it cannot place is a full run. What **is** a hard prerequisite is two affected cells. Fewer than two cannot support a broad-group claim and are reported as such; health controls do not count toward that number. `benchmarks/src/comparability.mjs` carries both the scenario selection and the role map in measurement identity, so neither a scoped/unscoped pair nor two differently attributed runs can be compared as though they measured the same estimand.
...
**The `steps` declarations are checked, not trusted.** `benchmarks/src/step-audit.mjs` first records direct public step calls, then rebuilds each distinct schema/context with every other registered plugin omitted in turn. If the one-step omission breaks construction, that step is an indirect dependency even when no method call exposed it — `templateLiteral` reaches `literal` this way through `v.union(['px', 'em', 'rem'])`. The declared set must equal direct plus omission-proven indirect dependencies exactly: a missing name would under-report impact/coverage, and an extra name would fabricate coverage. `pnpm bench:coverage` runs this exact audit before counting a declaration, and `pnpm --dir benchmarks verify` runs it again beside semantic verification. The exhaustive audit covers 114 registered steps and is construction-only; on the current 241-scenario catalog it takes about half a second locally.
...
**The impact gate shards four ways.** It used to say it was deliberately not sharded, and both of that claim's objections were checked rather than argued. The first was that the severe-*group* trigger becomes a cross-runner aggregate, and it was right — the answer given here before was wrong, so it is worth saying what the error was. Pairing cancels machine *speed* for each cell, because both sides of a cell are measured on one machine. It does **not** establish that the cell's candidate/base *ratio* is invariant across microarchitectures, cache topology, or frequency behaviour. And because a cell keeps its shard for all five repetitions, any such runner-dependent effect on its ratio is a **fixed effect** across repetitions: it shifts every `G_r` by the same amount and contributes zero variance, so the group's Student-t interval cannot widen for it. The previous claim that a shard's machine "cannot inflate the spread" is therefore the reason the group interval can be overconfident rather than a reassurance — the estimate can be tight and displaced at the same time. Not hypothetical: in run 30547023911 screen shard 2 ran on an AMD EPYC 9V74 while shards 0, 1 and 3 ran on AMD EPYC 7763. So the sharding stays and the **decision boundary** moved: a group trigger no longer blocks on the sharded screen alone. `confirm-measure` remeasures the whole triggered group on one runner and the group blocks only if that independent batch agrees — the standard a severe cell already had, two measurements against two independent runner draws — and where the batch does not fit one runner inside its budget the trigger is reported as `review`, with the arithmetic shown, because blocking on a cross-shard aggregate is blocking on evidence that cannot support it. The second was that the fixed cost does not shard, so the saving is well under 1/N; measured, the previous unsharded run spent 55m12s measuring against about 40s on checkout, setup, both builds, and scoping, so wall time is `40s + measurement/N` and four shards is about 28.5 minutes. Sharding is also the only answer that scales, because the cell count grows with every step added while a timeout is a constant that gets consumed. The merge happens before the verdict, so every group aggregate is computed once over the complete cell set, and `compare` still refuses to pair runs whose shard count or isolation differs, so sharding one side only fails loudly. Sharding by *repetition* rather than by cell stays rejected: it would put the repetitions of one cell on different machines, which is precisely what the paired design exists to avoid.

**The arithmetic behind blocking versus review.** Run 30547023911 measured 613 cell-runs per screen shard in an average of 1429 s, so one cell-run costs 2.33 s and a confirmation batch of N cells costs 23.3·N seconds, plus about two minutes of checkout, setup and building both revisions. The budget is 45 of the job's 60 minutes — the last quarter left deliberately, so that a runner slower than the one those figures came from turns a blocking decision into a slower verdict rather than a timeout that reads as infrastructure failure. That allows roughly 110 cells. `warm/failure/all` is 9 cells (3.5 min) and `warm/async/success` 12 (4.7 min), so both are confirmed outright; `warm/success` at 124 cells is 48.2 min of measurement by itself and therefore falls back to `review` whenever it triggers on an unscoped run. The fallback is forced by that arithmetic, not chosen for convenience, and `benchmarks/src/confirmation.mjs` holds the constants with the same derivation beside them.
...
Valchecker before/after uses **adjacent per-cell paired independent processes**. Inside one repetition the two builds for a cell run back to back before the runner moves to the next cell; odd repetitions run candidate then baseline and even repetitions reverse that order. This replaced the misleading "interleaved" workflow that actually ran an entire candidate shard before an entire baseline shard. The raw schema records `temporalPairing: 'adjacent-cell'`, so a historical whole-side result cannot be compared to the new measurement as if exposure to temporal drift were unchanged.
...
- an interval spanning a threshold: `inconclusive` — not a regression verdict, named in the report, and the input to the confirmation stage; the required-check resolver refuses success if the final answer remains inconclusive
...
| severe | inconclusive | `unresolved` | not a regression verdict; the required check does not pass without an answer |
| inconclusive | severe | `unresolved` | not a regression verdict; the required check does not pass without an answer |
...
The table is **symmetric in the two stages**: what decides is how many of them judged the row and whether they agree, not which one did. The first version read only the confirmation batch, and the gate's first real run showed the cost — `set/collect-all` came back inconclusive in the screen and severe at −30.2% in the confirmation batch and resolved as `reproduced`, while the same pair in the other order resolved as `unresolved`. One severe judgement against one non-judgement is the same evidence either way round. Only a **severe** claim produces a regression failure, which is the product rule that failed it before this stage existed; a reproduced plain regression is a review, as it was. Separately, the required-check invocation uses `--require-resolved`, so a final `unresolved` or `inconclusive` answer exits non-zero without being relabelled a regression.

**Accepted regressions are declared, not suppressed.** `benchmarks/src/accepted-regressions.mjs` holds an entry per cell the repository has decided to pay for: the cell id, how far down is accepted, and prose saying what the cost bought. The verdict reads it, so an acknowledged row stops blocking — and it is printed in the report with its measured depth, its bound, and its reason, because a gate whose passing output hides what it forgave is the failure mode this mechanism would otherwise become. A measurement deeper than the bound fails with both numbers. The list is checked for rot in both directions: an entry whose cell the screen now reports `cleared` fails, so the list shrinks as the code improves rather than quietly absorbing the next regression, and an entry naming a cell the catalog no longer declares fails in `pnpm bench:cells`, where the catalog alone settles it without waiting for a measurement. A **group** aggregate is forgiven only by an entry naming the group, in the same file and with the same rules, because "did this cell regress?" and "did this affected group broadly regress?" are different questions. `warm/failure/all` carries one such entry, bounded at 12%: the group measured −6.40% [−7.1%, −5.7%] and that number is true — two of its nine cells carry an accepted cost, `set/collect-all` alone accounting for −4.25pp and `map/collect-all` for −0.88pp — so the bound says how much of a true number was agreed to rather than hiding it. Leaving acknowledged cells out of the aggregate was rejected: it would condition the estimate on what was previously forgiven, which is the bias the group estimator was rebuilt to remove, and it would shrink the denominator so a new regression in that group is diluted instead of caught. A group entry additionally fails once none of its member cells is acknowledged, so it cannot outlive the reason it was written for. What a group bound cannot decide is whether the group's cost is still the accepted one — only that its magnitude is within a number a person set, which is why the reason records each acknowledged cell's contribution for a reader to subtract. Staleness is decided from the screen rather than from the confirmation batch, whose clearing of a cell the screen called severe is this gate's noise diagnostic. What it cannot decide is stated in the module: it cannot tell an accepted cost from a regression someone got tired of.

**Retiring an acknowledgement needs the same evidence as blocking on one.** An entry is stale only when two measurements agree the cost is gone: for a cell, the screen and the confirmation batch; for a group, a single-runner confirmation. The reason is empirical. The same group was reported by three consecutive hosted runs as −3.93% inconclusive, −6.40% regression with an interval of [−7.1%, −5.7%], and −3.44% cleared — and under the interval rule `cleared` and `regression` cannot both describe one quantity, so the between-run variation exceeds what the within-run interval captures. That is the between-runner fixed effect, demonstrated by this list's own rot check rather than argued. If a cross-shard screen cannot support blocking a group it cannot support un-acknowledging one either; otherwise the entry can neither exist nor not exist without failing on some runs, which is a gate no author can satisfy. So an acknowledged cell and an acknowledged group are both **always queued for the confirmation batch**, which is what keeps their entries falsifiable, and where the evidence is missing the check is reported as **unassessed** — neither a pass nor a failure, because an unassessed check that reads as "checked and fine" is the failure this mechanism exists to avoid. One consequence worth stating: a group too large to confirm on one runner inside the budget cannot have its acknowledgement assessed on this hardware at all, so for such a group `review` is the honest treatment rather than a bounded acknowledgement.

**An acceptance whose bound cannot be judged stays an acceptance.** A cell or group with an entry is exempt from the ordinary rules whatever its bound turned out to say; only the acceptance list can fail on it — a reproduced breach, a stale entry, a malformed one. Exempting only when the bound came out `within` meant that a bound the run could not judge dropped the row back to the ordinary 5% threshold, which is *stricter* than the bound the acknowledgement raised it to, so having the entry was worse than not having it. An `unassessed` acceptance is review, and it is named in the report.

**The static catalog audit reports every measurement-contract change it can read.** The apparatus comes from the candidate ref and therefore supplies `group`, `batch`, and `async` to both measured sides; runtime comparison cannot reconstruct an old value for any of them. The base/head AST audit now treats all three as the cell's measurement contract and reports `added / removed / contract changed`, naming each changed field with its before/after value. `group` changes aggregation, `batch` changes how many logical operations one timed wrapper represents, and `async` changes the execution path. An unreadable declaration fails closed rather than being presented as unchanged.

**A bound is a decision threshold and is judged like one.** It classifies each stage's *interval* against `-maxRegressionPercent` — wholly past it is a breach, wholly inside is within bound, spanning it is `unassessed` — and a breach blocks only when both batches reproduce it. It used to compare the deeper of the two point estimates, which reintroduced both failure modes the rest of the gate was hardened against: with a 45% bound a noisy screen at −60% followed by a confirmation `cleared` at 0% was reported as a breach and failed the workflow, so *adding* an acknowledgement made the gate stricter than having none, while a −40% point estimate whose interval ran past −45% was accepted although the measurement could not place the cost inside the bound. Rechecked when the semantics changed: every interval any run produced still sits inside its bound — map's widest is [−18.2%, −10.9%] against 25%, the group's is [−7.1%, −5.7%] against 12%, and set's widest is [−42.9%, +26.1%] against 45% by only 2.1pp — so the numbers set from point estimates survive, with the consequence that a noisier run reports `set/collect-all` `unassessed` rather than acknowledged. That is honest and blocks nothing, and the bound is deliberately not widened to make it read as acknowledged.

**Group confirmation is scheduled independently of row confirmation.** Each triggered group gets its own single-runner batch sized by the **affected cells that formed the screen estimator**; health canaries are not reintroduced during confirmation. Ordinary row confirmations keep their sharded batch and acknowledged cells ride with them. This keeps the second measurement about the same estimand as the first while preserving the earlier fix that unrelated boundary rows cannot consume a group's confirmation budget.

Nothing is pooled. Re-running an unsettled cell for k more repetitions and judging the combined sample is optional stopping however carefully the rule is pre-declared, because the set being extended is chosen by the first result; two batches of a fixed size have no such property.

**A triggered group is confirmed too, on one runner.** A severe group verdict blocks, so the confirmation batch remeasures every **affected estimator member** of that group — not the health controls and not merely the boundary rows selected for ordinary row confirmation — unsharded on one runner. The group blocks only if that independent batch also classifies the same affected estimator as a regression. The artifacts carry the role map, and `confirmation.mjs` verifies the batch measured the whole affected set; a sharded batch or one missing an affected member is review, never blocking evidence.
...
`serialization/*` measures the native JSON text conversions. `toJSONValue` remains a `compatible-subset` pair: Valchecker exposes `JSON.parse` as a built-in while the Zod spelling is a transform closure around the same native call. `toJSONString` now follows native `JSON.stringify()` success semantics directly, but its schema-level comparison is also `compatible-subset`: D7's executable `equivalent` contract includes a representative failure, and the cyclic case is observably different because Valchecker and Valibot catch the native `TypeError` while both Zod pins let the callback exception escape `safeParse`. The cyclic timing row itself remains `library-defaults` and gates out those Zod adapters. The old path-aware loss-preventing preflight is now the separate `toStrictJSONString()` step. No pinned competitor exposes that contract, so it is intentionally benchmark-coverage-exempt rather than compared against a hand-written stand-in.
...
### Executable equivalence conformance

`comparisonScope: equivalent` is an executable claim. Scenarios sharing a Valchecker build key and issue-policy context share a library-neutral conformance contract assembled from their correctness fixtures: at least one successful case with an **explicit output property** and one representative failure. Each participating adapter runs those same cases through the scenario's own entry point before the timed operation is created. A wrong result therefore prevents the row from existing; it cannot survive as a fast number with a misleading label.

Output expectation uses property presence rather than `expected.output !== undefined`, so `{ success: true, output: undefined }` is a real assertion. `schema-kind/undefined-valid` pins that distinction. The catalog records the contract id and case count, generated Markdown/HTML reports print them on equivalent rows, and a row with a material implementation difference carries an **Execution-model note**. Internal shape is deliberately not an equivalence authority: a callback, built-in validator, fused implementation, delegation primitive, or generated validator may differ while the observable contract remains the same. If the common success/failure/output contract cannot be expressed — as for the maybe-async transform failure — the scenario is `compatible-subset` instead.
...
`pnpm bench:coverage` (`scripts/check-benchmark-coverage.ts`, part of `pnpm test:quality` and therefore of `pnpm verify`) turns that field into a gate only **after** executing the exact step audit above. It then reads the built-in names from each step's `Meta.Name` and the covered set from the now-proven scenario catalog, and fails when a built-in is named by no compared scenario or when a scenario names no built-in. It does not need `benchmarks/node_modules`; it does need the current Valchecker build so the construction evidence describes the code being counted. `pnpm verify` already builds before `test:quality`, and a standalone coverage run without a usable build fails closed instead of printing unaudited coverage.
...
The gate's allowlist is for steps no pinned competitor can express, so no comparison exists to build, and each entry carries the verified reason. It currently holds five: `as`, which is a compile-time cast whose implementation is `noop` and installs no runtime step at all; `isIncludingKey` and `isIncludingValue`, because no competitor has a Map membership check — Zod 3 exports no `includes`, and Zod 4's and Valibot's throw on a Map because they reach `Array.prototype.includes` — so the only competitor spelling is a hand-rolled `refine` closure, which for `isIncludingValue` would also have to reimplement the step's SameValueZero match; `json`, whose scenarios exist but gate out all four competitors for the reason under `JSON string validation` below; and `toStrictJSONString`, whose path-aware loss-preventing preflight has no pinned competitor equivalent while native stringify is already compared through `toJSONString`. `toSafeNumber` was on this list until 2026-07-28 on a reason that turned out to be false, and is now measured. The list cannot rot in either direction: an entry for a step some scenario now compares against a competitor fails, so it shrinks as the suite grows, and an entry for a step that no longer exists fails too.
...
`src/step-recorder.mjs` loads the published `allSteps` set and `createValchecker()`, keeps one stable proxy for the adapter, and switches that proxy between the full instance and cached instances with exactly one plugin removed. Returned schema proxies unwrap every argument before forwarding it, so Valchecker itself never receives the audit proxy. `src/step-audit.mjs` uses that instrument to discover direct **and indirect** dependencies and requires exact set equality. This is why `templateLiteral` can honestly declare `literal` without turning all extra declarations into an escape hatch: omitting `literal` breaks the build, while omitting a fabricated `isIncludingKey` does not.
...
- `serialization/json-value-*` — `toJSONValue()` differs only by a Valchecker built-in against a competitor closure around the same native parse call. The `toJSONString()` rows are no longer in this list: native stringify success is `equivalent` and its reported failure is `library-defaults`.
...
- `async/transform-valid` — the successful async transformation is shared, but a non-string rejected before Valchecker's async callback returns synchronously while the competitor async schemas still require their async entry point. There is no shared failure execution model, so the row cannot satisfy an `equivalent` success+failure contract.
- `issue-policy/loose-object/invalid/{first,all}` — the timed failure fixtures agree on issue-count policy, but the successful passthrough output has observably different property enumeration order: Valchecker emits the undeclared key before the declared keys while the competitors preserve the input's `first`, `second`, `extra` order.

`union-large/*`, `recursion/*`, `nullish/*`, `schema-kind/*`, `standard-schema/*`, `primitive/*`, `primitive-builtin/valid`, `flat-object/valid`, `flat-object-builtin/valid`, `nested-object/*`, `delegation/valid`, and the async check rows are `equivalent` by observable contract. The label no longer means that every library uses the same internal primitive: each participating adapter must first pass the same executable conformance cases — exact success output plus representative failure where one exists — through that scenario's own native or Standard Schema entry. `any` and `unknown` are the deliberate exception to the failure requirement because no rejecting JavaScript value exists; they instead run multiple exact preserved-output successes. Material implementation differences such as Valchecker's `check()` closure versus competitor regex actions, or `use(inner)` versus `pipe(inner)`, are printed as an **Execution-model note** rather than used to weaken semantic equivalence.
