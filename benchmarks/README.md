...
`serialization/*` measures the native JSON text conversions. `toJSONValue` remains the `compatible-subset` pair: Valchecker exposes `JSON.parse` as a built-in while the Zod spelling is a transform closure around the same native call. `toJSONString` now follows native `JSON.stringify()` semantics directly, so `serialization/json-string-valid` is `equivalent`; the cyclic invalid row is `library-defaults`, because Valchecker and Valibot catch the same native `TypeError` but report their own issue shapes, while both Zod pins let the callback exception escape `safeParse` and are gated out. The old path-aware loss-preventing preflight is now the separate `toStrictJSONString()` step. No pinned competitor exposes that contract, so it is intentionally benchmark-coverage-exempt rather than compared against a hand-written stand-in.
