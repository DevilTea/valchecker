...
For a built-in step, `scripts/check-step-completeness.ts` (`pnpm steps:complete`) enforces the part of that list a script can decide, and it reports every problem for every step at once: the [step unit](.claude/skills/valchecker-dev/references/step-unit.md)'s file set, naming patterns and section order, a colocated `<name>.test.ts` registering at least one `it` or `test`, a `<name>.bench.ts` calling `bench`, an export reaching `api-surface.json`, a `<name>.doc.md` whose `### ` entry writes the step's name in call form in a code span, describes it, and holds a `ts` example, and every owned issue code both listed in a code span of that entry and present in a string in one of the directory's tests. HTML comments and fenced code blocks do not count as documentation, and a comment does not count as a test.
...
Several of those rules are weaker than the requirement they stand for, and their messages say so: a registered test case may assert nothing, a string holding an issue code may never reach an assertion, an entry mentioning `toTrimmedStart()` may be saying it does not exist, a helper module need only be imported rather than used, and the section order cannot decide which section a local *type* belongs to. They catch a piece going missing, not a piece going wrong — review still has to read what was written.
