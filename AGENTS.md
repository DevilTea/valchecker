...
A normal built-in step uses `Meta` for public name, expected current state, and owned issue type; `PluginDef` for the state-aware signature and canonical JSDoc; and `implStepPlugin()` for runtime registration and operation mode. Those three names are fixed, its directory holds one implementation, one runtime test, one benchmark and a one-line `index.ts`, and the sections inside `<name>.ts` run in one order. That is [the step unit](.claude/skills/valchecker-dev/references/step-unit.md), and `pnpm steps:complete` enforces it; see also [architecture](.claude/skills/valchecker-dev/references/architecture.md).
...
- the step's `<name>.doc.md`, from which `pnpm docs:api:update` regenerates `docs/api/*`;
- READMEs, the hand-written VitePress pages, the page templates under `scripts/docs-api-templates/`, and the skills under `.claude/skills/`;
- `CHANGELOG.md`, and `MIGRATION.md` when the change is breaking.

For a built-in step, `scripts/check-step-completeness.ts` (`pnpm steps:complete`) enforces the part of that list a script can decide, and it reports every problem for every step at once: the [step unit](.claude/skills/valchecker-dev/references/step-unit.md)'s file set, naming patterns and section order, a colocated `<name>.test.ts` with a module-scope `it` or `test` imported from Vitest, a `<name>.bench.ts` calling the imported `stepBench`, an export reaching `api-surface.json`, a `<name>.doc.md` whose opening `### ` heading writes the step's name in call form in a code span, describes it, and holds a non-empty `ts` example, and every owned issue code both listed as a complete token in a code span of that entry and present as a complete token in a string in one of the directory's tests. HTML comments and fenced code blocks do not count as documentation, and a comment does not count as a test.

`docs/api/*` is generated from those entries and the page templates under `scripts/docs-api-templates/`, by `scripts/docs-api.ts`. `pnpm docs:api` fails when a committed page stops matching its sources and `pnpm docs:api:update` rewrites them, the same pair as `pnpm api:surface`. Never hand-edit a page under `docs/api`: it carries a banner saying so. A step declares which page it lands on (`category`) and which section of it (`section`); neither is inferred from its name, and an unknown category, a section no template offers, a section no step fills, or a step with no `.doc.md` all fail rather than silently taking the step off the site.

Several of those rules are weaker than the requirement they stand for, and their messages say so: a registration-shaped call may sit behind arbitrary control flow the gate does not interpret, a registered test case may assert nothing, a string holding an issue code may never reach an assertion, an entry describing `toTrimmedStart()` may say something false, a helper module need only be imported rather than used, and the section order cannot decide which section a local *type* belongs to. They catch a piece going missing, not a piece going wrong — review still has to read what was written.
