---
name: maintain-docs
description: Maintain Valchecker's documentation architecture, narrative pages, checked examples, generated API reference, route compatibility, visuals, and source-backed semantic review. Use for documentation changes and whenever source changes may affect documented behavior.
---
...
- [`references/content-architecture.md`](./references/content-architecture.md) — section boundaries, page ownership, route compatibility, and reader flow;
- [`references/writing-guidelines.md`](./references/writing-guidelines.md) — prose, source linkage, examples, visuals, and generated-artifact rules;
- [`docs/_meta/pages.ts`](../../../docs/_meta/pages.ts) — canonical narrative page identity, order, and deterministic per-page requirements;
- [`docs/_meta/compatibility.ts`](../../../docs/_meta/compatibility.ts) — canonical ownership for preserved legacy routes and their final dispositions.
...
- Published legacy routes live in `docs/_meta/compatibility.ts`; generated compatibility pages under old paths must not be hand-edited.
...
Behavioral narrative claims use `relatedSources` for the smallest exact source roots that directly define the claim. Do not pad it with barrels, package roots, directories, generated files, or every transitive helper.

## Workflow

1. **Load contracts.** Read `AGENTS.md`, this skill, both references, the page registry, the route-compatibility mapping, and any local canonical owner relevant to the task.
2. **Classify ownership.** Decide whether each change belongs to a step `.doc.md`, narrative page, registry, route mapping, checked fixture, API template, or generated artifact before editing.
...
4. **Edit canonical owners only.** Never patch generated API or compatibility output to make a check pass.
5. **Regenerate derived artifacts.** Use the established update command (`pnpm docs:api:update`, `pnpm docs:compat:update`, or another canonical generator) when its owner changes.
...
- `pnpm docs:compat` — non-mutating legacy-route mapping and generated compatibility-artifact check.
- `pnpm docs:compat:update` — regenerate old published routes from `docs/_meta/compatibility.ts`.
...
- **mechanically stale/broken** — automation can prove the defect, for example a missing `relatedSources` path, generated API/compatibility mismatch, invalid page contract, broken include/asset, failing checked example, or failed production build.
...
The reviewer is read-only. It should look for source/prose contradictions, claims that are too broad or too narrow, duplicate ownership, misleading examples, inaccurate visuals, missing impacted pages, route-compatibility mistakes, and migration/troubleshooting drift. It should not spend its budget re-running a known-passing full gate by default.
