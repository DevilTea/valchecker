...
- Runtime-sensitive narrative example code → canonical checked fixture once #149 establishes `docs/.examples`.
...
During #148–#154, legacy `docs/guide/*` and `docs/examples/*` pages may remain registered with `transitional: true`. This is an inventory bridge only: it preserves deterministic navigation and H1 identity while content is re-homed by reader need.

A transitional page is not exempt from existence/H1 checks, but it may defer final page-local source metadata until its migration issue rewrites it into a canonical destination. The final cleanup must leave no legacy page merely because it was once transitional.
