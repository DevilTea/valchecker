...
Each measurement takes between `minSamples` and `maxSamples` samples and stops as soon as its 95% confidence interval is within `targetRelativeMarginOfError` of the mean, so different cells in one run stand on different numbers of samples and the reported RME says what each achieved. The interval uses Student's t, which is what makes the target mean what it says at these sample sizes. `pnpm --dir benchmarks test` checks the rule, and CI runs it in the `Benchmark-Smoke` job.

The target is 0.75%, set against the 440 cells of the 2026-07-27 full run: replaying that run under the rule reproduces the complete ranking of all 95 scenarios with a worst-case shift of 1.21% in any reported ratio, while taking 61% of the samples. Loosening it to 1.5% changes scenario winners. Only the `full` profile saves much — its 750 ms samples are long enough to settle early, whereas a `standard` 300 ms sample is noisy enough that even a trivial operation rarely reaches the target before the seven-sample cap. Changing any profile field changes what a number means, so `compare` refuses to pair runs whose profiles differ rather than reporting the difference as a performance change.
