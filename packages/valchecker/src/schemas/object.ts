...
type InferObjectOutput<O extends Record<PropertyKey, ValSchema>> = {
	[K in keyof O]: InferOutput<O[K]>
}

export class ObjectSchema<O extends Record<PropertyKey, ValSchema>> extends AbstractSchema<ObjectSchemaTypes<O>> {}
...
implementSchemaClass(
	ObjectSchema,
	{
		...
		validate: (value, { meta, success, failure, createExecutionChain }) => {
			...
			return chain.then<ValidationResult<Record<PropertyKey, any>>>(() => issues.length === 0 ? success(value) : failure(issues))
		},
	},
)

export function object<O extends Record<PropertyKey, ValSchema>>(struct: O, message?: ObjectSchemaMessage<O>): ObjectSchema<O> {
