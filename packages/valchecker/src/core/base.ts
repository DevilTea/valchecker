...
type AbstractBaseSchemaConstructorPayload<T extends SchemaTypes> = Simplify<
...
abstract class AbstractBaseSchema<T extends SchemaTypes = any> extends NullProtoObj implements StandardSchemaV1<T['input'], T['output']> {
	...
	constructor(
		...params: IsAllPropsOptional<AbstractBaseSchemaConstructorPayload<T>> extends true
			? [payload?: AbstractBaseSchemaConstructorPayload<T>]
			: [payload: AbstractBaseSchemaConstructorPayload<T>]
	) {
		super()
...
type InferTransformed<Schema extends (ValSchema | AbstractBaseSchema)> = NonNullable<Schema['~standard']['types']>['transformed']
...
type InferValidateReturn<Schema extends (ValSchema | AbstractBaseSchema)>
...
| (false extends InferAsync<Schema>
	...
	: never)
type InferIsValidReturn<Schema extends (ValSchema | AbstractBaseSchema)>
= (true extends InferAsync<Schema>
	? Promise<ValidationResult<InferOutput<Schema>>>
	: never)
| (false extends InferAsync<Schema>
...
type ValSchema<Input = any, Output = Input> = AbstractBaseSchema<{
	readonly async: any
...
type UntransformedValSchema<Input = any, Output = Input> = AbstractBaseSchema<{
	readonly async: any
