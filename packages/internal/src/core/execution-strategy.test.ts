...
describe('operation-mode execution contracts', () => {
	it('keeps every synchronous execution entrypoint synchronous', () => {
		...
		expect(schema.execute('value'))
			.toEqual({ value: 'value' })
		expect(schema['~standard'].validate('value'))
			.toEqual({ value: 'value' })
	})
	...
	it('keeps maybe-async execution adaptive to the actual result', async () => {
		...
		expect(synchronous['~core'].operationMode)
			.toBe('maybe-async')
		expect(synchronous['~execute']('value'))
			.toEqual({ value: 'value' })
		expect(synchronous.execute('value'))
			.toEqual({ value: 'value' })
		expect(synchronous['~standard'].validate('value'))
			.toEqual({ value: 'value' })
