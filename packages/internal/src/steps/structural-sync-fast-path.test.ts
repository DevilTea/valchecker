...
describe('synchronous structural child result handling', () => {
	it('skips thenable inspection for collection children', () => {
		...
		expect(v.array(doubled).execute([1, 2]))
			.toEqual({ value: [2, 4] })
		expect(v.set(doubled).execute(new Set([1, 2])))
			.toEqual({ value: new Set([2, 4]) })
		expect(v.map({ key: identity, value: doubled }).execute(new Map([['a', 1]])))
			.toEqual({ value: new Map([['a', 2]]) })
	})

	it('skips thenable inspection for object children', () => {
		const upper = createSynchronousFixture(value => String(value).toUpperCase())
		const input = { value: 'a', extra: true }

		expect(v.object({ value: upper }).execute(input))
			.toEqual({ value: { value: 'A' } })
		expect(v.looseObject({ value: upper }).execute(input))
			.toEqual({ value: { value: 'A', extra: true } })
		expect(v.strictObject({ value: upper }).execute({ value: 'a' }))
			.toEqual({ value: { value: 'A' } })
	})
	...
	it('skips thenable inspection for combinator branches', () => {
		...
		expect(v.union([identity]).execute('value'))
			.toEqual({ value: 'value' })
		expect(v.intersection([identity, identity]).execute('value'))
			.toEqual({ value: 'value' })
		expect(v.variant({
			discriminator: 'type',
