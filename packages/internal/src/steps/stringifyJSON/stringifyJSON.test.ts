...
describe('stringifyJSON plugin', () => {
	describe('valid inputs', () => {
		it('should stringify object', () => {
			...
			const result = v.stringifyJSON().execute(obj)
			expect(result).toEqual({ value: '{"name":"John","age":30}' })
		})

		it('should stringify array', () => {
			const arr = [1, 2, 3]
			const result = v.stringifyJSON().execute(arr)
			expect(result).toEqual({ value: '[1,2,3]' })
		})

		it('should stringify string', () => {
			const result = v.stringifyJSON().execute('hello')
			expect(result).toEqual({ value: '"hello"' })
		})

		it('should stringify number', () => {
			const result = v.stringifyJSON().execute(42)
			expect(result).toEqual({ value: '42' })
		})

		it('should stringify boolean', () => {
			const result = v.stringifyJSON().execute(true)
			expect(result).toEqual({ value: 'true' })
		})

		it('should stringify null', () => {
			const result = v.stringifyJSON().execute(null)
			expect(result).toEqual({ value: 'null' })
		})

		it('should stringify undefined', () => {
			const result = v.stringifyJSON().execute(undefined)
		...
		it('should stringify empty object', () => {
			const result = v.stringifyJSON().execute({})
		...
		it('should stringify nested structure', () => {
			...
			const result = v.stringifyJSON().execute(nested)
			expect(result).toEqual({ value: '{"user":{"name":"John"},"items":[1,2]}' })
		})
	})
	...
	describe('invalid inputs', () => {
		it('should fail for function', () => {
			const func = () => {}
			const result = v.stringifyJSON().execute(func)
			expect(result).toEqual({
				issues: [{
					code: 'stringifyJSON:unserializable',
					payload: { value: func },
		...
		it('should fail for Symbol', () => {
			...
			const result = v.stringifyJSON().execute(sym)
			expect(result).toEqual({
				issues: [{
					code: 'stringifyJSON:unserializable',
					payload: { value: sym },
		...
		it('should fail for BigInt', () => {
			const result = v.stringifyJSON().execute(123n)
			expect(result).toEqual({
				issues: [{
					code: 'stringifyJSON:unserializable',
					payload: { value: 123n },
	...
	describe('custom messages', () => {
		it('should use custom message for unserializable value', () => {
			const result = v.stringifyJSON('Custom error message').execute(() => {})
			expect(result).toEqual({
				issues: [{
					code: 'stringifyJSON:unserializable',
