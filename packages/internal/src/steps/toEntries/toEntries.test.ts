...
describe('toEntries step plugin', () => {
	it('returns Map entries in insertion order as new tuple arrays', () => {
		...
		const result = v.map({ key: v.string(), value: v.number() }).toEntries().execute(input)

		expect(result).toEqual({ value: [['b', 2], ['a', 1]] })
	...
	it('infers mutable key-value tuples', () => {
		...
		expectTypeOf<InferOutput<typeof schema>>().toEqualTypeOf<Array<[string, number]>>()
	})
})
