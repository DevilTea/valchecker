...
const setFixture = implStepPlugin<any>({
...
describe('set step plugin', () => {
	it.each([
		...
		expect(v.set(v.string()).execute(value)).toEqual({
			issues: [{
	...
	it('uses a custom message for its owned classification issue', () => {
		expect(v.set(v.string(), { message: 'Set required' }).execute([])).toMatchObject({
	...
	it('returns transformed items in insertion order without mutating the input', () => {
		...
		const schema = v.set(v.string().transform(value => value.toUpperCase()))

		expect(schema.execute(input)).toEqual({ value: new Set(['A', 'B']) })
	...
	it('applies the parent message handler to nested child issues', () => {
		...
		expect(schema.execute(new Set<unknown>([1]))).toMatchObject({
			issues: [{
	...
	it('rejects transformed item collisions instead of silently deduplicating items', () => {
		...
		const schema = v.set(
			v.string().transform(value => value.toLowerCase()),
			{ message: issue => `set:${issue.code}` },
		)
	...
	it('uses SameValueZero when detecting transformed item collisions', () => {
		const schema = v.set(v.string().transform(value => value === 'first' ? Number.NaN : Number.NaN))

		expect(schema.execute(new Set(['first', 'second']))).toMatchObject({
			issues: [{ code: 'set:duplicate_transformed_item', path: [1] }],
		})
	})

	it('preserves fully synchronous collection execution', () => {
		const result = v.set(v.string()).execute(new Set(['a']))

		expect(result).not.toBeInstanceOf(Promise)
		expect(result).toEqual({ value: new Set(['a']) })
	})
	...
	it('stops later items after a synchronous internal child failure', () => {
		...
		expect((v as any).set(item).execute(new Set(['internal', 'later']))).toMatchObject({
			issues: [{
	...
	it('stops later items after an asynchronous internal child failure', async () => {
		...
		await expect((v as any).set(item).execute(new Set(['internal', 'later'])))
			.resolves.toMatchObject({
				issues: [{
					code: 'core:unknown_exception',
					category: 'internal',
