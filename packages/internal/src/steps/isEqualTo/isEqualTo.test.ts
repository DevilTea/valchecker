...
describe('isEqualTo step plugin', () => {
	...
	it('narrows primitive and mixed primitive-object unions', () => {
		...
		expectTypeOf<InferOutput<typeof schema>>()
			.toEqualTypeOf<'ready'>()
		expect(schema.execute('ready'))
			.toEqual({ value: 'ready' })

		const mixed = v.union([v.string(), v.object({ value: v.number() })])
			.isEqualTo('ready')
		expectTypeOf<InferOutput<typeof mixed>>()
			.toEqualTypeOf<'ready'>()
		expect(mixed.execute({ value: 1 }))
			.toMatchObject({ issues: [{ code: 'isEqualTo:expected_equal_to' }] })
	})

	it('supports unknown output but excludes initial and object-only states', () => {
		const schema = v.unknown()
			.isEqualTo(true, { message: 'Expected true' })
		expectTypeOf<InferOutput<typeof schema>>()
			.toEqualTypeOf<true>()
		expect(schema.execute(false))
			.toMatchObject({ issues: [{ message: 'Expected true' }] })
		if (false) {
			// @ts-expect-error isEqualTo is unavailable in the initial state
			v.isEqualTo(true)
			v.object({})
				// @ts-expect-error isEqualTo is unavailable for object-only output
				.isEqualTo(true)
		}
		v.unknown()
