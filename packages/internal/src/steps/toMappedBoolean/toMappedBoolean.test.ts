...
describe('toMappedBoolean step plugin', () => {
	...
	it('uses SameValueZero equality', () => {
		...
		expect(schema.execute(Number.NaN))
			.toEqual({ value: true })
		expect(schema.execute(0))
			.toEqual({ value: false })
	})
	...
	it('snapshots mappings when the schema is created', () => {
		...
		const schema = v.string()
			.toMappedBoolean({ trueValues, falseValues })

		trueValues.push('later-true')
		falseValues.push('later-false')

		expect(schema.execute('maybe'))
			.toMatchObject({
				issues: [{
					code: 'toMappedBoolean:unmapped_value',
					payload: {
	...
	it('supports custom messages and one-sided mappings', () => {
		...
		expect(schema.execute('enabled'))
			.toEqual({ value: true })
		expect(schema.execute('disabled'))
			.toMatchObject({
	...
	it('infers boolean output and follows the current primitive type', () => {
		...
		if (false) {
			// @ts-expect-error toMappedBoolean is unavailable once the output is already boolean
			v.boolean().toMappedBoolean({}) // eslint-disable-line style/newline-per-chained-call -- single line keeps the directive covering the whole unreachable negative-type expression
		}

		if (false) {
			// @ts-expect-error mappings must use the current string output type
			v.string().toMappedBoolean({ trueValues: [1], falseValues: [0] }) // eslint-disable-line style/newline-per-chained-call -- single line keeps the directive covering the whole unreachable negative-type expression
		}
	})
})
