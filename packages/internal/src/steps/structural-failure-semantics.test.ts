...
describe('structural requiredness contract', () => {
	for (const [name, createSchema] of variants) {
		it(`${name} reports a missing own key instead of validating synthetic undefined`, () => {
			const result = createSchema(v.number()).execute({})
		...
		it(`${name} validates an own property whose value is undefined`, () => {
			const result = createSchema(v.number()).execute({ value: undefined })
			expect(result).toEqual({
				issues: [{
	...
	it('skips an absent optional child but emits an own undefined output property', () => {
		for (const schema of [
			...
			expect(result).toEqual({ value: { value: undefined } })
			if (v.isSuccess(result))
				expect(Object.hasOwn(result.value, 'value')).toBe(true)
		}
	})

	it('does not treat an inherited property as present', () => {
		const input = Object.create({ value: 1 })
		expect(v.object({ value: v.number() }).execute(input))
	...
	it('supports symbol keys and points missing-key paths at the symbol', () => {
		...
		expect(v.object({ [key]: v.number() }).execute({ [key]: 1 }))
			.toEqual({ value: { [key]: 1 } })
		expect(v.object({ [key]: v.number() }).execute({}))
			.toEqual({
				issues: [{
	...
	it('continues structural requiredness checks after an asynchronous child', async () => {
		const first = v.number().transform(async value => value)
	...
	it('reads an enumerable own getter once per object execution', () => {
		...
		for (const schema of [
			...
			expect(schema.execute(input)).toEqual({ value: { value: 1 } })
...
describe('fatal and recoverable propagation contract', () => {
	...
	it('nested unions preserve inner-to-outer provenance and dynamic message metadata', () => {
		const result = v.union([
			v.union([
				v.string(({ context }) => `inner:${String(context?.length)}`),
			]),
			v.number(),
		]).execute(null)
		expect(result).toMatchObject({
	...
	it('object and array stop evaluating siblings/items after an internal issue', () => {
		...
		const objectResult = v.object({
			fatal: v.fatal(),
			later: v.unknown().transform(sibling),
		}).execute({ fatal: 1, later: 2 })
		expect(objectResult).toMatchObject({ issues: [{ code: 'fatal:failed', path: ['fatal'] }] })
		expect(sibling).not.toHaveBeenCalled()

		const itemRun = vi.fn()
		const arrayResult = v.array(v.fatal(itemRun)).execute([1, 2, 3])
		expect(arrayResult).toMatchObject({ issues: [{ code: 'fatal:failed', path: [0] }] })
	...
	it('object variants and arrays stop after an asynchronous internal issue', async () => {
		for (const createSchema of [
			(later: () => void) => v.object({
				first: v.unknown().transform(async value => value),
				fatal: v.fatalAsync(),
				later: v.unknown().transform((value) => { later(); return value }),
			}),
			...
			const later = vi.fn()
			const result = await createSchema(later).execute({ first: 1, fatal: 2, later: 3 })
			expect(result).toMatchObject({
		...
		const itemRun = vi.fn()
		const result = await v.array(v.fatalAsync(itemRun)).execute([1, 2])
		expect(result).toMatchObject({ issues: [{ code: 'fatalAsync:failed', path: [0] }] })
		expect(itemRun).toHaveBeenCalledTimes(1)
	})

	it('union handles asynchronous recoverable and internal branch failures', async () => {
		const recoverable = () => v.unknown().transform(async () => { throw new Error('recoverable') })
...
describe('intersection conflict contract', () => {
	it('reports the nested conflict path and branch pair', () => {
		const result = v.intersection([
			v.unknown().transform(() => ({ user: { id: 1 } })),
			v.unknown().transform(() => ({ user: { id: 2 } })),
		]).execute(null)
		expect(result).toEqual({
			issues: [{
