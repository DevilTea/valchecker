...
describe('use step plugin', () => {
	it('returns the delegated schema output', () => {
		...
		expect(v.unknown().use(delegated).execute('  ADA@EXAMPLE.COM  ')).toEqual({
			value: 'ada@example.com',
		})
	})

	it('passes through delegated issues without replacing their contract', () => {
		expect(v.unknown().use(v.number()).execute('wrong')).toMatchObject({
			issues: [{
	...
	it('composes multiple delegated schemas in chain order', () => {
		const normalizeCase = v.string().toLowercase()
		const normalizeSpace = v.string().toTrimmed()
	...
	it('returns a native Promise only when delegated work becomes asynchronous', async () => {
		...
		const result = v.unknown().use(delegated).execute('ada')
	...
	it('passes through asynchronous delegated failures', async () => {
		const delegated = v.string().check(async value => value.length > 5 || 'Too short')
		const result = v.unknown().use(delegated).execute('hi')
	...
	it('does not execute the delegated schema after an earlier failure', () => {
		...
		const delegated = v.number().transform(callback)

		expect(v.string().use(delegated).execute(42)).toMatchObject({
