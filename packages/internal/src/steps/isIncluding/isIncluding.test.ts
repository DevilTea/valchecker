...
describe('isIncluding step plugin', () => {
	...
	it('uses native Set SameValueZero membership', () => {
		...
		expect(v.set(v.number()).isIncluding(Number.NaN).execute(nanSet)).toEqual({ value: nanSet })
		expect(v.set(v.number()).isIncluding(-0).execute(zeroSet)).toEqual({ value: zeroSet })
	})

	it('reports the Set payload variant and custom message', () => {
		const value = new Set([1])
		expect(v.set(v.number()).isIncluding(2, { message: 'Missing Set item' }).execute(value)).toEqual({
			issues: [{
