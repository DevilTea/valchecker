...
Root exports include the state-aware plugin types and `implStepPlugin()` used to define custom fluent methods. `StepPluginImpl<Def>` is the implementation shape accepted by `implStepPlugin()`; its registered `StepPlugin<Def>` return type is the value accepted by `createValchecker({ steps })`. Use the repository's [Custom Step Plugins guide](https://deviltea.github.io/valchecker/extending/custom-step-plugins) for the current `Meta`, `PluginDef`, trailing-options, issue-draft, operation-mode, testing, and tree-shaking contracts.
