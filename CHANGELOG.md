...
- The `${number}` and `${bigint}` template-literal grammars have one definition each, owned by `looseNumber` and `looseBigint` and shared with `templateLiteral`'s placeholders, replacing a regex that was duplicated verbatim between `looseBigint.ts` and `template-literal-part.ts` and two spellings of the same number rule. Both cite tsc's `isValidBigIntString`/`isValidNumberString`, so they were one rule written twice; a test now pins that a placeholder and the corresponding schema accept exactly the same strings.
...
- **Breaking (edge):** `isJwt()` now applies the base64url length rule to all three segments, not just their alphabet. A base64url string never has a length of `1 (mod 4)`, because a final group of one character cannot encode a byte, so tokens with such a header, payload, or signature are now rejected; `isBase64Url()` already rejected those strings, and the two steps disagreed. Header and payload must still be non-empty and an empty signature is still accepted, both unchanged. The rule now has one definition shared by both steps.
