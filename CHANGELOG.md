...
- `isEmoji({ registered: true })` opts in to Unicode's RGI set, the sequences vendors are expected to render. It compiles `\p{RGI_Emoji}` through `new RegExp` inside a module-level capability probe rather than a regular-expression literal, and where the runtime has no `v` flag it fails with the new operation issue `isEmoji:unsupported_registered_set` (payload `{ value, error }`) instead of falling back. There is deliberately no fallback: an accepted set that depends on the runtime is a wrong answer that is harder to notice than a failure, which is the whole reason the option exists.

### Fixed

- `isEmoji()` no longer accepts a lone `Emoji_Component` on either accepted set. `\p{RGI_Emoji}` matches the five skin-tone modifiers (`🏻`–`🏿`) and the four hair components (`🦰`–`🦳`) as single characters, so the previous implementation accepted `🏽` as an emoji, and accepted `👪🏻` and `👍🏽🏽` by decomposing them into a registered emoji plus a bare component. The registered path now matches `[\p{RGI_Emoji}--\p{Emoji_Component}]`. A component is still accepted where the grammar gives it a position: `👨‍🦰` and `👍🏽` are unaffected. Verified by enumerating the registered set out of the `v`-flag sub-properties — 3,646 members across `Basic_Emoji`, `Emoji_Keycap_Sequence`, `RGI_Emoji_Modifier_Sequence`, `RGI_Emoji_Flag_Sequence`, `RGI_Emoji_Tag_Sequence`, and `RGI_Emoji_ZWJ_Sequence` — where the nine lone components are the only members either path now rejects.
