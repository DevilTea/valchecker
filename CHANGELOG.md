...
- `isIsoDate()` and `isIsoDateTime()` validate the calendar with a single pattern instead of round-tripping through `Date`. Month lengths and the leap-year rule are encoded in the shared date source, so a success no longer allocates a `Date` or parses four numbers. Validating a date-time measured 298 ns before and 35 ns after (2026-07-27); `string-format/iso-date-time-valid` was the largest gap in the cross-library suite at 3.89x behind the fastest peer. The accepted set is unchanged, including the pre-existing rejection of `0000-02-29`: the two implementations agree on 25,512 generated inputs covering every day of every month across leap years, leap centuries and the `0000`-`0099` range, every hour, minute and second, and every offset and fractional-second shape.
