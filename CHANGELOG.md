...
- `isIsoTime()` validates field ranges as part of its pattern instead of comparing parsed numbers after a shape match, and the time grammar now has one definition shared with `isIsoDateTime()`'s time and offset halves. Measured: a valid time 106.9 ns to 16.2 ns (6.6x), a fractional time 6.4x, an out-of-range field 5.7x, a malformed string 1.4x — every path improves, since the time grammar has no alternation to walk. `isIsoDateTime`'s composed pattern source is byte-identical to before, and `isIsoTime`'s accepted set is unchanged across an exhaustive run of all 1,000,000 `HH:MM:SS` field combinations plus fractional and malformed shapes.
