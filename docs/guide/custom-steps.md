...
```ts
const positiveNumber = v.number().check(
  value => value > 0,
...
// Usage
v.number().divisibleBy(3)  // Must be divisible by 3
```
...
// Usage: string → string[]
const schema = v.string().toSplitArray(',')
...
describe('positive step', () => {
  describe('valid inputs', () => {
    it('should pass for positive numbers', () => {
      const schema = v.number().positive()
      expect(schema.run(1)).toEqual({ isOk: true, value: 1 })
      expect(schema.run(0.5)).toEqual({ isOk: true, value: 0.5 })
      expect(schema.run(Number.MAX_VALUE)).toEqual({ isOk: true, value: Number.MAX_VALUE })
    })
  })
  ...
  describe('invalid inputs', () => {
    it('should fail for zero', () => {
      ...
      if (!result.isOk) {
        expect(result.issues[0].code).toBe('positive:expected_positive')
      }
    })
  ...
  describe('custom messages', () => {
    it('should use custom message when provided', () => {
      const schema = v.number().positive('Must be positive!')
    ...
    it('should support message function', () => {
      ...
      const result = schema.run(-5)
      if (!result.isOk) {
        expect(result.issues[0].message).toBe('-5 is not positive')
      }
    })
  })
})
...
```ts
// Good: Focused steps
v.string().toTrimmed().min(1).max(255)
...
export const isbn = implStepPlugin<PluginDef>({
  isbn: ({
    ...
    addSuccessStep((value) => {
      const isValid =
        format === '10' ? isValidISBN10(value)
        : format === '13' ? isValidISBN13(value)
